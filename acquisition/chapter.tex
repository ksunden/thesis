\chapter{Acquisition} \label{cha:acq}
...
This chapter details several iterations of data acquisition software used by the Wright Group.
The first, PyCMDS, is a program originally written by Dr. Blaise Thompson.
Upon joining the group, I took over some of the maintainership of this program, and moved to primary maintainer upon Dr. Thompson's graduation.
Next, PyCMDS was modified to use \yaq{} for hardware control, and was given a new name, \yaqccmds{}.
Dr. Thompson did help with the transition, though continued maintenance remained under my purview.
Finally, we entered into a collaboration with people running many large scale facilities to use Bluesky.
Bluesky is a project that was originally developed at Brookhaven National Lab for controlling the National Synchrotron Light Source II.
While many parts of the Bluesky system, including the core ``Run Engine'' and the ``Queue Server'' are written and maintained by external organizations, many projects required particular attention to adapt these powerful tools to the Wright Group.
In particular, I (with consultation with Dr. Blaise Thompson) introduced \wrightplans{}, \biab{}, and \blueskycmds{}.
\wrightplans{} provides specialized experimental procedures used by the Wright Group.
\biab{} bundles services written elsewhere into a single application that can be run more easily.
Additionally, it adds writing of WrightTools data files from acquisitions taken using Bluesky.
\blueskycmds{} provides a graphical user interface for interacting with the upstream-provided Queue Server.
...
Virtual hardware was implemented to allow for development of PyCMDS away from the active instruments, but even then it made some development tasks arduous.
...
\newglossaryentry{bluesky-cmds}{name=bluesky-cmds, description={Graphical application for interacting with a Bluesky Queueserver}}
\newglossaryentry{Queueserver}{name=Queueserver, description={A background service which runs Bluesky plans in a queue}}
\newglossaryentry{Bluesky-in-a-box}{name=Bluesky-in-a-box, description={A Docker compose application which runs all services needed to run a Queueserver}}
\newglossaryentry{Docker}{name=Docker, description={A system to run applications in a reproducible way}}
\newglossaryentry{yaqc-qtpy}{name=yaqc-qtpy, description={An application to interact with \yaq{} daemons which is built on traits and serves engineering purposes}}
\newglossaryentry{yaqd-control}{name=yaqd-control, description={A command line application to manage \yaq{} daemons}}
\newglossaryentry{yaq-traits}{name=yaq-traits, description={A command line application to generate Avro Protocols from more human friendly TOML files}}
\newglossaryentry{Avro-protocol}{name={Avro Protocol (avpr)}, description={A complete listing of all messages suported by an avro RPC}}
\newglossaryentry{CLI}{name=CLI, description={Command Line Interface, an application that is run in a text-based terminal}}
\newglossaryentry{RPC}{name=RPC, description={Remote Procedure Call}}
\newglossaryentry{trait}{name={trait (yaq)}, description={A collection of messages, config, state, and properties that provides consistent interaction with multiple daemons}}
\newglossaryentry{property}{name={property (yaq)}, description={A collection of messages which provide access to one logical value, such as getter and setter, along with associated messages such as units and options}}
\newglossaryentry{daemon}{name={daemon}, description={A small background program that performs a single task}}
\newglossaryentry{OPA}{name={OPA}, description={Optical Parametric Amplifier, a device which changes the wavelength of light by adjusting motors controlling crystal and grating angles and delay times}}
...
\newglossaryentry{plan}{name=plan, description={A Python function which defines a series of steps of an experimental procedure}}
...
Do \inlinebash{docker compose up --build} to run the Docker containers.
...
It is provided by the project itself, and serves as a valuable tool for both troubleshooting and as an ``escape hatch'' for things that more narrowly focussed interfaces do not make possible.
